...
//				std::cout << "Deleting data: " << m_name << std::endl;
		...
		std::string AsText()
		{
			std::string str(m_data, m_data + m_data_sz);
			return str;
		}
		...
		Data Transpose(int size, unsigned int &blocksz)
		{
			...
			for( int j = 0 ; j < size ; j += 1 )
				for( int i = 0 ; i < m_data_sz ; i += size )
					temp[end++] = m_data[i + j];
...
double KeySizeNormalisation(Data &cipher, unsigned int keysize, int rounds=6)
{
	using namespace std;
	if (keysize == 0)
	{
...
std::string DecryptWithGuess(Data &data, unch guess, int start, int end, bool onlyascii = true)
{
	std::string txt;
...
static bool sortFunc(std::pair<unch, double> const& p1,  std::pair<unch, double> const& p2)
{
	return p1.second > p2.second;
}

#define THRESHOLD 0.85
std::vector<std::pair<unch, double> > DecryptCeaserBlock(Data &data, int start, int blksz)
{
	using namespace std;
	std::vector<std::pair<unch, double> > char_prob;
	string freqs = /*" eEaAoOiItTuUhH\n" + */ AllAsciiString();
	unch most = data.MostFrequentByte(start, blksz);
	for(unsigned int i = 0 ; i < freqs.length() ; i += 1 )
	{
		unch freq = freqs[i];
		unch guess = freq ^ most;

		string txt = DecryptWithGuess(data, guess, start, start + blksz);
		if (txt.length() == 0)
			continue;
		...
		{
//			string outstring = Int2Str((int) guess, true); // + string(txt);
//			cout << "::::::: " << outstring << "    :" << probability << endl;
			char_prob.push_back(std::make_pair(guess, probability));
		}
	}
	std::sort(char_prob.begin(), char_prob.end(), sortFunc);
	return char_prob;
}

std::vector<std::vector<std::pair<unch, double> > > DecryptCeaserRepeat(Data &data, int blksz)
{
	using namespace std;
	std::vector<std::vector<std::pair<unch, double> > > mymaps;
	string freqs = /* " eEaAoOiItTuUhH" + */ AllAsciiString();
	blksz = (blksz == 0) ? data.Size() : blksz; // if zero, take full.

	for( int offset = 0 ; offset < data.Size() ; offset += blksz )
	{
		std::vector<std::pair<unch, double> > char_probs = DecryptCeaserBlock(data, offset, blksz);
		mymaps.push_back(char_probs);
	}
	return mymaps;
}

unch * CollectCharsForKey(std::vector<std::vector<std::pair<unch, double> > > blockkey_coll)
{
	if (blockkey_coll.size() == 0)
		return NULL;

	unch * secret = new  unch[blockkey_coll.size()];
	int index = 0;

	std::vector<std::vector<std::pair<unch, double> > >::iterator it;
	for(it = blockkey_coll.begin(); it != blockkey_coll.end(); ++it)
	{
		std::vector<std::pair<unch, double> > block_choices = *it;
		if (block_choices.size() > 0)
			secret[index++] = block_choices[0].first;
		else
			secret[index++] = '\0';
	}
	return secret;
...
bool DecryptRepeatXOR(Data &cipher)
{
	using namespace std;
	unsigned int keysizeguess = 0; // FindKeySize(cipher) | get largest.;
    ...
    unsigned int blocksize = 0;
		...
		vector<vector<pair<unch, double> > > guesses = DecryptCeaserRepeat(transpose, blocksize);
		unch * secret = CollectCharsForKey(guesses);
		Data key = Data(secret, keysizeguess);
		delete secret;

		Data realkey   = Data("Terminator X: bring the noise", PLAINTEXT);
		cout << "Hamming distance: " << HammingDist(key, realkey) << endl;

//		Data result = cipher.EncryptWithKey(realkey);
//		cout << result.AsText() << endl;
	}
	return true;
}

Data ReadDataFromFile(std::string filename)
{
	using namespace std;
	...
	string line;
	inf >> line;
	Data content = Data(line, BASE64STR);
	return content;
}
...
bool Problem06()
{
	...
	Data cipher = ReadDataFromFile(string("problem06.dat"));
...
bool DecryptCeaser(std::string hexData)
{
	using namespace std;
	if (hexData.length() == 0)
		return false;
	Data satz = Data(hexData, HEXSTRING);
	vector<pair<unch, double> > guesses =  DecryptCeaserBlock(satz, 0, satz.Size());
	...
	vector<pair<unch, double> >::iterator itx;
	for(itx = guesses.begin() ; itx != guesses.end() ; ++itx)
	{
		cout << itx->second << " => " << itx->first << " :" ;
		unch uch = itx->first;
		Data key = Data(&uch, 1);
		Data plaintext = satz.EncryptWithKey(key);
		cout << plaintext.AsText() << endl;
		break;
	}
...
{
	using namespace std;
	DecryptCeaser("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736");
	return true;
}
