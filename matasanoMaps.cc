...
//				int s = end / size;
//				int rend = (b * size) + s;
//				std::cout << end << ":" << b << " " << s << " " << rend << std::endl;
//				temp[end] = (rend < m_data_sz) ? m_data[rend] : 0;
//				end++;
//			}
			
//			if (m_data_sz % size)
//				std::cout << "Direct transposing poses extra" << std::endl;
...
std::map<std::string, double> DecryptCeaserBlock(Data &data, int start, int blksz)
{
	using namespace std;
...
std::map<std::string, double> DecryptCeaserRepeat(Data &data, int blksz)
{
	using namespace std;
	map<string, double> mymap;
	...
	{
//		cout << "Block: " <<  offset/blksz << endl;
		map<string, double> strings = DecryptCeaserBlock(data, offset, blksz);
		mymap.insert(strings.begin(), strings.end());
	}
	return mymap;
}
...
{
	using namespace std;
	unsigned int keysizeguess = 0; // FindKeySize(cipher);
	...
	for( int i = 29 ; i < 30 ; i += 1 )
	{
		...
		map<string, double> strings = DecryptCeaserRepeat(transpose, blocksize);
...
bool DecryptCeaser(std::string hexData)
{
	...
	map<string, double> strings = DecryptCeaserRepeat(satz, 0);
	map<string, double>::iterator it;
	if (strings.size() <= 0) 
		return false;
